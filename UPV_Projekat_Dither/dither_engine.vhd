...
architecture dither_engine_arch of dither_engine is
	...
	subtype word_t is signed(1 downto 0);	--5o bitni podaci
	...
	signal r_row, g_row, b_row : memory_t := (others => (others => '0'));
...
begin
	...
	r_acum_error <= r_row(position)&"000" + r_error; --6 bitna stvar za racun, 5 bita nadalje
	g_acum_error <= g_row(position)&"000" + g_error; 
	b_acum_error <= b_row(position)&"000" + b_error; 
			...
			case next_state is
				...
				when WAIT_ROW =>
					...
					if (current_state = WAIT_PIXEL) then  
						r_row(CAM_WIDTH-1) <= r_error(4 downto 3);
						g_row(CAM_WIDTH-1) <= g_error(4 downto 3);
						b_row(CAM_WIDTH-1) <= b_error(4 downto 3);
					end if;
				when PROCESS_PIXEL =>
					...
					if (current_state = WAIT_PIXEL) then
						r_row(position - 1) <= r_back(5 downto 4); --pazi na duzine,nizi bit je bitan za racun ali ne i za 
						g_row(position - 1) <= g_back(5 downto 4);
						b_row(position - 1) <= b_back(5 downto 4);
					end if;
